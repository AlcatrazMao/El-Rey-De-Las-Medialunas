import { RefreshCw } from 'lucide-react';
import * as React from 'react';
import { useState } from 'react';

import type { AppSettings, SyncSettings as SyncSettingsType } from '../hooks/useSettings';

interface Props {
  settings: AppSettings;
  onUpdate: (section: 'sync', values: Partial<SyncSettingsType>) => void;
  onSaved: () => void;
}

export const SyncSettings: React.FC<Props> = ({ settings, onUpdate, onSaved }) => {
  const [form, setForm] = useState<SyncSettingsType>({ ...settings.sync });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onUpdate('sync', form);
    onSaved();
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="flex items-center gap-2 border-b border-gray-100 dark:border-zinc-800 pb-4">
        <RefreshCw className="h-4 w-4 text-amber-500" />
        <h3 className="text-sm font-extrabold text-gray-800 dark:text-zinc-50">Sincronización</h3>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="text-[10px] font-extrabold text-gray-500 uppercase tracking-wider">
            Retención de datos locales (días)
          </label>
          <input
            type="number"
            min="1"
            max="90"
            step="1"
            value={form.cleanupDays}
            onChange={e => setForm(f => ({ ...f, cleanupDays: Number(e.target.value) }))}
            className="w-full text-xs font-semibold bg-gray-50 dark:bg-zinc-850 border border-gray-200 dark:border-zinc-700 rounded-xl p-3 focus:outline-none focus:ring-1 focus:ring-amber-500 text-gray-850 dark:text-zinc-100"
          />
          <p className="text-[10px] text-gray-400">
            Ventas y movimientos más antiguos se limpian del almacén local después de este período.
          </p>
        </div>

        <div className="space-y-1">
          <label className="text-[10px] font-extrabold text-gray-500 uppercase tracking-wider">
            Sincronización automática
          </label>
          <label className="flex items-center gap-3 w-full text-xs font-semibold bg-gray-50 dark:bg-zinc-850 border border-gray-200 dark:border-zinc-700 rounded-xl p-3 cursor-pointer">
            <input
              type="checkbox"
              checked={form.autoSyncOnClose}
              onChange={e => setForm(f => ({ ...f, autoSyncOnClose: e.target.checked }))}
              className="h-4 w-4 accent-amber-500 cursor-pointer"
            />
            <span className="text-gray-850 dark:text-zinc-100">
              Sincronizar al cerrar caja
            </span>
          </label>
          <p className="text-[10px] text-gray-400">
            Al cerrar la sesión de caja, sube todos los datos pendientes al servidor.
          </p>
        </div>
      </div>

      <div className="pt-2">
        <button
          type="submit"
          className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl transition-all shadow-sm cursor-pointer"
        >
          Guardar cambios
        </button>
      </div>
    </form>
  );
};
